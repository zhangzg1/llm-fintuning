from peft_code.peft_util import process_data, load_model, load_dataset, test_with_batch
from peft_code.peft_lora import build_peft_model, build_train_arguments, build_trainer


def main():
    # 处理原始的训练数据，生成微调的训练数据集
    input_train_path = "dataset/ChineseMedical/medical_train.txt"
    output_train_path = "dataset/ChineseMedical/train_dataset.json"
    process_data(input_train_path, output_train_path)
    # 处理原始的评估数据，生成微调的评估数据集
    input_eval_path = "dataset/ChineseMedical/medical_eval.txt"
    output_eval_path = "dataset/ChineseMedical/eval_dataset.json"
    process_data(input_eval_path, output_eval_path)
    # 处理原始的测试数据，生成模型评测的测试数据集
    input_tset_path = "dataset/ChineseMedical/medical_test.txt"
    output_test_path = "dataset/ChineseMedical/test_dataset.json"
    ...
    lora_output_path = "saved/medical_lora_model"
    ...
    # model_lora = "models/Qwen2-7B-Instruct-lora"
    # model_lora_quantized = "models/Qwen2-7B-Instruct-lora-awq"
